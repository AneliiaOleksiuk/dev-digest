...
|---|---|---|---|---|---|
| [planner](planner.md) | Turns a feature/task request into a structured **Development Plan** grounded in real files, `AGENTS.md`/`INSIGHTS.md` constraints, do-not-touch paths, and the skills catalog. Never writes code, never executes the plan. | `Read`, `Grep`, `Glob`, `Bash` (read-only inspection only), `AskUserQuestion`, `Write` (scoped to `docs/plans/` only — no `Edit`) | `opus` | Task/feature request; root + package `AGENTS.md`/`INSIGHTS.md`; target module code; `.claude/skills/*/SKILL.md` catalog | Development Plan (chat report) **and** a saved copy at `docs/plans/<slug>.md` |
...
- `implementer`'s `permissionMode: default` asks for manual per-write
  ...
  (see [Sources](#sources-for-planner-and-implementers-rules) below).
- `planner` and `researcher` both gate on vagueness: an underspecified
  request triggers `AskUserQuestion` before any planning/searching starts.
...
Both personas' hard constraints are grounded in specific repo conventions
...
- `.claude/skills/*/SKILL.md` catalog — source for the "Skill catalog by
  ...
  name an exact skill.
- Canonical [`agents/planner.md`](../../agents/planner.md) — this file is a
  manual mirror of that source; divergence between them is a bug in the
  mirror, not an intentional difference.

**implementer** (`implementer.md`):
- Root [`AGENTS.md`](../../AGENTS.md) — same do-not-touch lists as
  `planner`; the "update `INSIGHTS.md` before ending a session" convention
  that drives the End-of-session step.
...
- The `planner` agent's Development Plan report format — `implementer`'s
  entire input contract (work items, `Applicable skills`, `Definition of
  done`) is defined by `planner`'s own report structure, not chosen
  independently.
...
- Canonical [`agents/implementer.md`](../../agents/implementer.md) — mirror
  source, same convention as `planner`.
