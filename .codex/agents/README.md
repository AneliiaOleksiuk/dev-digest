...
Six agents mirrored into three tools (`planner`, `implementer`, `researcher`,
`test-writer`, `plan-verifier`, `doc-writer`); see
...
from `.claude/agents/*.md` even though the underlying rule (e.g. "planner
may only write its own plan file") is identical — Codex enforces less
...
|---|---|---|---|---|---|
| [planner](planner.toml) | Same as canonical: Development Plan from a task request, grounded in `AGENTS.md`/`INSIGHTS.md`/skills catalog. Never edits code. | `workspace-write` — binary lever, so it's set one notch looser than "read-only" purely to allow the one file it must write; the `docs/plans/`-only scope is instruction-enforced, not sandboxed. | `model_reasoning_effort = "high"` | Task request; root + package `AGENTS.md`/`INSIGHTS.md`; skills catalog | Development Plan (chat) + `docs/plans/<slug>.md` |
...
| [plan-verifier](plan-verifier.toml) | Two-phase gate (spec compliance + architecture review): Phase 1 verifies per-item plan compliance against observable evidence with non-hedgeable verdicts; Phase 2 is the semantic judgment layer above `pnpm arch:check`, read-only. | `workspace-write` — same looks-looser-than-it-is situation `planner` already documents (test-runner caches, not source edits) | `model_reasoning_effort = "high"`, prefer a different model family than the implementer where possible | A Development Plan + code state; `implementer`'s report (for what to check only); `server/.dependency-cruiser.cjs` | Plan Verification report (Phase 1 traceability table + VERDICT, then Phase 2 Architecture Review findings table) — chat only |
...
- `plan-verifier` is `workspace-write` to allow test-runner caches even
  ...
  the same looks-looser-than-it-is situation `planner.toml` already
  documents. `researcher` is the only agent here where `sandbox_mode =
  "read-only"` enforces "no writes" at the OS level.
...
- `planner`'s `sandbox_mode = "workspace-write"` looks like it grants more
  than Claude's equivalent (`Write` scoped to `docs/plans/` only) — it
  doesn't. Codex has no path-scoped write permission, so the same
  `docs/plans/`-only boundary is enforced by `developer_instructions` alone
  here, not by the platform. Don't read the wider sandbox mode as a wider
  actual scope.
...
## Sources for planner's and implementer's rules
...
- `.claude/skills/*/SKILL.md` catalog — source of the "Skill catalog by
  domain" table in both `planner.toml` and `implementer.toml`. Codex has no
  native skills mechanism; the table exists purely so a human reading the
  plan can point Codex at the right skill file manually.
- `engineering-insights` skill — defines the `INSIGHTS.md` update `planner`
  flags and `implementer` performs at session end.
...
- Canonical [`agents/planner.md`](../../agents/planner.md) /
