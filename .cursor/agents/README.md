...
Six agents total in this mirror (same roster, same roles as the canonical
...
|---|---|---|---|---|---|
| [planner](planner.md) | Same as canonical: Development Plan from a task request, grounded in `AGENTS.md`/`INSIGHTS.md`/skills catalog. Never edits code. | `false` — binary lever, so it's set to allow the one file it must write; the `docs/plans/`-only scope is instruction-enforced, not platform-enforced. | `claude-opus-5[effort=high]` — pinned, since planning quality is this agent's bottleneck | Task request; root + package `AGENTS.md`/`INSIGHTS.md`; skills catalog | Development Plan (chat) + `docs/plans/<slug>.md` |
| [implementer](implementer.md) | Same as canonical: executes an approved plan, edits code, runs tests. | `false` — real write access, unlike planner/researcher's effective read-only-by-instruction/platform | `inherit` | A Development Plan (pasted or `docs/plans/*.md` path); package `INSIGHTS.md` | Code changes; test results; updated `INSIGHTS.md`; Implementation Report (chat) |
...
- `researcher` is the only agent here where `readonly: true` is
  **platform-enforced**, not instruction-only. `plan-verifier` is
  ...
  Claude Code, same shape `planner.md`'s comment already documents for its
  own scoped write.
- **`model` pinning has a documented caveat**: Cursor's forum reports that
  ...
  top of [`planner.md`](planner.md). Don't assume `opus` actually ran for
  `planner` without checking.
...
- `.claude/skills/*/SKILL.md` catalog — source of the "Skill catalog by
  domain" table in both `planner.md` and `implementer.md`. Cursor has no
  native skills mechanism; the table exists so a human reading the plan
  can point Cursor at the right skill file manually.
...
- `cursor.com/docs/subagents` — cited directly in
  [`planner.md`](planner.md)'s comment as the source for the
  `model: claude-opus-5[effort=high]` bracketed-options syntax, and for the
  documented full frontmatter field list (`name`, `description`, `model`,
  `readonly`, `is_background`) cited in
  [`implementer.md`](implementer.md)'s comment.
- Cursor's community forum — cited in [`planner.md`](planner.md)'s comment
  as the source for the "model field can be silently ignored" caveat and
  its reported v2.5 fix.
