...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean, highlighted = false): CSSProperties => {
    const edgeColor = focused || highlighted ? sevColor : "var(--border)";
    return {
      borderRadius: 8,
      // Fully longhand per side — `borderColor`/`borderWidth` are themselves
      // shorthands for all four sides, so pairing either with `borderLeft*`
      // is the same shorthand-vs-longhand conflict as `border`+`borderLeft`
      // (React warns "Updating a style property during rerender" on every
      // rerender that touches one of these, e.g. an accept/dismiss click).
      borderStyle: "solid",
      borderTopWidth: 1,
      borderRightWidth: 1,
      borderBottomWidth: 1,
      ...
      boxShadow: highlighted ? "0 0 0 3px " + sevColor : focused ? "0 0 0 1px " + sevColor : "none",
    };
  },
  header: {
    display: "flex",
