 ...
 *
 * Relocated here (from `server/src/adapters/git/diff-parser.ts`) so both the
 * server AND the pre-push CLI (`mcp/src/cli.ts`) share exactly one
 * implementation — see `reviewer-core/AGENTS.md`'s "no second implementation"
 * rule. `server/src/adapters/git/diff-parser.ts` now just re-exports this.
 */
